...
public class ArrayList<E> implements List<E> {
    ...
    private class MyListIterator implements ListIterator<E> { 
        private int j = 0;                   // index of the next element to report
        private boolean nextRemovable = false;   // can remove be called at this time?
        private boolean previousRemovable = false; // can remove be called at this time?
        ...
        public E next() throws NoSuchElementException {
            ...
            nextRemovable = true;   // this element can subsequently be removed
            previousRemovable =false;
            return data[j++];   // post-increment j, so it is ready for future call to next
        }
        ...
        @Override
        public void remove() throws IllegalStateException {
            if (!nextRemovable && !previousRemovable) {
                throw new IllegalStateException("nothing to remove");
            } else if (nextRemovable) {
                ArrayList.this.remove(j - 1);  // that was the last one returned
                j--;                         // next element has shifted one cell to the left
                nextRemovable = false;           // do not allow remove again until next or previous is called  
            }else{
                ArrayList.this.remove(j);  // that was the last one returned
                
                previousRemovable = false;           // do not allow remove again until next or previous is called  
            }
...
backwards.
        */
        ...
        public E previous() {
            ...
            previousRemovable = true;   // this element can subsequently be removed
            nextRemovable =false;
            return data[--j];   // pre-decrement j, so it is ready for future call to previous
        }
        /*
        ...
        */
        ...
        public void set(E obj) {
            if (!nextRemovable && !previousRemovable) {
                ...
                ArrayList.this.set(j - 1,obj);  // that was the last one returned               
            }else{
                ArrayList.this.set(j ,obj);  // that was the last one returned                
            }
        }
        /*
...
the iterator.
    ...
    public ListIterator<E> myListIterator(int i) {
        return new MyListIterator(i);    // create a new instance of the inner class
